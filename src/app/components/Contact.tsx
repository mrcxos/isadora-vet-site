...
export function Contact() {
  ...
  const inputStyle = { width: "100%", padding: "0.875rem 1rem", borderRadius: "12px", border: "1px solid rgba(28,25,23,0.12)", background: "rgba(255,255,255,0.8)", fontSize: "0.9375rem", color: "var(--color-text-dark)", outline: "none", transition: "border-color 0.2s, box-shadow 0.2s", fontFamily: "var(--font-sans)" };
  const labelStyle = { display: "block" as const, fontSize: "0.875rem", fontWeight: 600, color: "var(--color-text-dark)", marginBottom: "0.5rem" };
  const focusStyle = (e: React.FocusEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => { e.target.style.borderColor = "var(--color-primary)"; e.target.style.boxShadow = "0 0 0 3px rgba(196,98,45,0.12)"; };
  ...
  return (
    <section id="contato" className="py-28" style={{ background: "linear-gradient(180deg, #FFFFFF 0%, var(--color-bg-light) 100%)" }}>
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        <motion.div initial={{ opacity: 0, y: 32 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="text-center mb-20">
          <span className="inline-block px-4 py-1.5 rounded-full text-xs font-semibold tracking-widest uppercase mb-5" style={{ background: "rgba(196,98,45,0.10)", color: "var(--color-primary)" }}>{contact.badge_text}</span>
        ...
        <div className={contact.show_form !== false ? "grid lg:grid-cols-5 gap-12 lg:gap-16 items-start" : "max-w-2xl mx-auto"}>
          <motion.div initial={{ opacity: 0, x: -32 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className={contact.show_form !== false ? "lg:col-span-2" : ""}>
            <h3 className="mb-8" style={{ fontWeight: 600, color: "var(--color-text-dark)", fontSize: "1.25rem", lineHeight: 1.3 }}>{contact.sidebar_title}</h3>
            ...
            <a href={`https://wa.me/${contact.whatsapp_number.replace(/\D/g, '')}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-4 p-5 rounded-2xl mb-4 transition-all duration-200 hover:-translate-y-0.5 group" style={{ background: "rgba(37,211,102,0.08)", border: "1px solid rgba(37,211,102,0.18)", textDecoration: "none" }}>
              ...
              <div>
                <p style={{ fontWeight: 600, color: "var(--color-text-dark)", fontSize: "0.9375rem" }}>WhatsApp</p>
            ...
            <a href={`mailto:${contact.email}`} className="flex items-center gap-4 p-5 rounded-2xl mb-4 transition-all duration-200 hover:-translate-y-0.5" style={{ background: "rgba(196,98,45,0.06)", border: "1px solid rgba(196,98,45,0.14)", textDecoration: "none" }}>
              <div className="w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0" style={{ background: "rgba(196,98,45,0.10)" }}><Mail size={22} style={{ color: "var(--color-primary)" }} /></div>
              <div>
                <p style={{ fontWeight: 600, color: "var(--color-text-dark)", fontSize: "0.9375rem" }}>E-mail</p>
                <p style={{ color: "#78716C", fontSize: "0.875rem" }}>{contact.email}</p>
                <p style={{ color: "#78716C", fontSize: "0.8125rem" }}>{contact.email_response_text}</p>
              </div>
            </a>
            ...
            <a href={contact.instagram_url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-4 p-5 rounded-2xl mb-4 transition-all duration-200 hover:-translate-y-0.5" style={{ background: "rgba(193,53,132,0.06)", border: "1px solid rgba(193,53,132,0.14)", textDecoration: "none" }}>
              ...
              <div>
                <p style={{ fontWeight: 600, color: "var(--color-text-dark)", fontSize: "0.9375rem" }}>Instagram</p>
                <p style={{ color: "#C13584", fontSize: "0.875rem" }}>{contact.instagram_handle}</p>
                <p style={{ color: "#78716C", fontSize: "0.8125rem" }}>{contact.instagram_subtitle}</p>
              </div>
            </a>
            ...
            <a href={`tel:${contact.phone_number.replace(/\s/g, '')}`} className="flex items-center gap-4 p-5 rounded-2xl transition-all duration-200 hover:-translate-y-0.5" style={{ background: "rgba(107,124,92,0.06)", border: "1px solid rgba(107,124,92,0.14)", textDecoration: "none" }}>
              <div className="w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0" style={{ background: "rgba(107,124,92,0.10)" }}><Phone size={22} style={{ color: "var(--color-secondary)" }} /></div>
              <div>
                <p style={{ fontWeight: 600, color: "var(--color-text-dark)", fontSize: "0.9375rem" }}>Telefone</p>
            ...
            <div className="mt-8 p-5 rounded-2xl" style={{ background: "linear-gradient(135deg, rgba(196,98,45,0.06), rgba(107,124,92,0.06))", border: "1px solid rgba(196,98,45,0.12)" }}>
              <p style={{ fontSize: "0.875rem", color: "#57534E", lineHeight: 1.6 }}>
                🐾 <strong style={{ color: "var(--color-text-dark)" }}>{contact.trust_badge_text}</strong>
              </p>
            </div>
          </motion.div>

          {contact.show_form !== false && <motion.div initial={{ opacity: 0, x: 32 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="lg:col-span-3">
            <div className="p-8 lg:p-10 rounded-3xl" style={{ background: "rgba(255,255,255,0.9)", backdropFilter: "blur(16px)", border: "1px solid rgba(196,98,45,0.12)", boxShadow: "0 8px 40px rgba(196,98,45,0.08)" }}>
              {submitted ? (
                <div className="flex flex-col items-center justify-center py-16 text-center gap-5">
                  <div className="w-16 h-16 rounded-full flex items-center justify-center" style={{ background: "rgba(196,98,45,0.12)" }}><CheckCircle2 size={32} style={{ color: "var(--color-primary)" }} /></div>
                  <h3 style={{ fontFamily: "var(--font-display)", fontSize: "1.5rem", fontWeight: 600, color: "var(--color-text-dark)" }}>{contact.success_title}</h3>
                  <p style={{ color: "#78716C", fontSize: "1rem", lineHeight: 1.7, maxWidth: "360px" }}>{contact.success_body}</p>
                </div>
              ) : (
                <form onSubmit={handleSubmit}>
                  <h3 className="mb-2" style={{ fontWeight: 700, color: "var(--color-text-dark)", fontSize: "1.25rem" }}>{contact.form_title}</h3>
                  <p className="mb-8" style={{ color: "#78716C", fontSize: "0.9rem" }}>{contact.form_subtitle}</p>
                  ...
                  <button type="submit" className="w-full flex items-center justify-center gap-2.5 py-4 rounded-2xl text-base font-medium mt-6 transition-all duration-300 hover:shadow-xl hover:-translate-y-0.5 active:scale-95" style={{ background: "linear-gradient(135deg, var(--color-primary), var(--color-primary-dark))", color: "white", boxShadow: "0 4px 20px rgba(196,98,45,0.35)" }}>
                    <Send size={18} />{contact.submit_button_text}
                  </button>
                  <p className="text-center mt-4" style={{ fontSize: "0.8125rem", color: "#78716C" }}>{contact.privacy_text}</p>
                </form>
              )}
            </div>
          </motion.div>}
