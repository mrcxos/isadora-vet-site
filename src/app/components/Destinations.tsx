...
import { useDestinations, useSectionContent } from "../../hooks/useSiteData";

const difficultyColor = {
  "Simples":  { text: "#16A34A", bg: "rgba(22,163,74,0.10)" },
  "Moderado": { text: "var(--color-primary)", bg: "rgba(196,98,45,0.10)" },
  "Rigoroso": { text: "#DC2626", bg: "rgba(220,38,38,0.10)" },
};

export function Destinations() {
  const { data: destinations } = useDestinations();
  const { data: section } = useSectionContent('destinations');

  return (
    <section id="destinos" className="py-28" style={{ background: "linear-gradient(180deg, #FFFFFF 0%, var(--color-bg-light) 100%)" }}>
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
              ...
              >
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <span style={{ fontWeight: 600, color: "var(--color-text-dark)", fontSize: "0.9375rem" }}>{dest.country}</span>
                  </div>
                ...
                <ul className="flex flex-col gap-1.5">
                  {dest.requirements.map(req => (
                    <li key={req} className="flex items-start gap-2">
                      <div className="w-1 h-1 rounded-full mt-2 flex-shrink-0" style={{ background: "var(--color-primary)" }} />
        ...
        <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: 0.2 }} className="mt-14 p-8 rounded-2xl text-center" style={{ background: "linear-gradient(135deg, rgba(196,98,45,0.06), rgba(107,124,92,0.06))", border: "1px solid rgba(196,98,45,0.14)" }}>
          ...
          <button onClick={() => document.querySelector("#contato")?.scrollIntoView({ behavior: "smooth" })} className="inline-flex items-center gap-2 text-sm font-semibold transition-all hover:gap-3" style={{ color: "var(--color-primary)" }}>
            {section.secondary_cta_text} <ArrowRight size={16} />
          </button>
        </motion.div>
      </div>
    </section>
